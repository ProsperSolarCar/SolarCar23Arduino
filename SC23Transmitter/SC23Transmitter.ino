...
// GPS Pins
// Pins on the Arduino that are wired to the GPS's TX and RX pins.
#define GPS_PIN_TX 7 // blue wire.  GPS RX
#define GPS_PIN_RX 8
...
void setup ()
  {
  iLoopCounter = 0;
  iRadioLoopFreq; // every iRadioLoopFreq loops, send a radio packet
...
void updateGps ()
  {
  // TODO: Do we need to limit the GPU data read per loop?
  ...
  if (gps.speed.isUpdated())
    {
    dSpeedMph   = gps.speed.mph();
    strSpeedMph = String (dSpeedMph, 2);
    }
  }
...
void updateSensors ()
  {
  //Serial.print(":00714F"); // has to be this address for the data radio to know who it is!
  fAuxVoltage = (analogRead(1)-515) / 4;
  fSolarCurrent = (analogRead(2)-515) / 4;
  fMotorCurrent = 0;//(analogRead(3)-515) / 3;
  fMainVoltage = ((analogRead(5)* (5.0/1023) * ((10000+570)/570))-4.1);

  strAuxVoltage   = String (fAuxVoltage, 2);   // max 20
  strSolarCurrent = String (fSolarCurrent, 2); // max 20
  strMotorCurrent = String (fMotorCurrent, 2); // max 100
  strMainVoltage  = String (fMainVoltage, 2);  // max 90
  }
...
void displayGpsInfo()
  {
  // Debug code for testing the gps chip
  ...
  if (gps.date.isValid())
    ...
    Serial.print("Mph:" + (String)gps.speed.mph());
    }
  else
    {
    Serial.print(F("INVALID"));
